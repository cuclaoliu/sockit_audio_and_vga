...
#      in the nios_core_ddr3_p0_timing.tcl script.
#    * The helper routines are defined in nios_core_ddr3_p0_pin_map.tcl
#
# NOTE
# ----
...
source "$script_dir/nios_core_ddr3_p0_timing.tcl"
...
set half_period [ nios_core_ddr3_p0_round_3dp [ expr $t(CK) / 2.0 ] ]

# Half of reference clock
set ref_half_period [ nios_core_ddr3_p0_round_3dp [ expr $t(refCK) / 2.0 ] ]
...
set data_output_min_delay [ nios_core_ddr3_p0_round_3dp [ expr - $t(wru_output_min_delay_external) - $t(wru_output_min_delay_internal)]]
...
set data_output_max_delay [ nios_core_ddr3_p0_round_3dp [ expr $t(wru_output_max_delay_external) + $t(wru_output_max_delay_internal)]]
...
set data_input_max_delay [ nios_core_ddr3_p0_round_3dp [ expr $t(rdu_input_max_delay_external) + $t(rdu_input_max_delay_internal) ]]
...
set data_input_min_delay [ nios_core_ddr3_p0_round_3dp [ expr - $t(rdu_input_min_delay_external) - $t(rdu_input_min_delay_internal) ]]

# Minimum delay on address and command paths
set ac_min_delay [ nios_core_ddr3_p0_round_3dp [ expr - $t(IH) -$fpga(tPLL_JITTER) - $fpga(tPLL_PSERR) - $board(intra_addr_ctrl_skew) + $board(addresscmd_CK_skew) - $ISI(addresscmd_hold) ]]

# Maximum delay on address and command paths
set ac_max_delay [ nios_core_ddr3_p0_round_3dp [ expr $t(IS) +$fpga(tPLL_JITTER) + $fpga(tPLL_PSERR) + $board(intra_addr_ctrl_skew) + $board(addresscmd_CK_skew) + $ISI(addresscmd_setup) ]]
...
if { ! [ info exists nios_core_ddr3_p0_sdc_cache ] } {
	set nios_core_ddr3_p0_sdc_cache 1
	nios_core_ddr3_p0_initialize_ddr_db nios_core_ddr3_p0_ddr_db
} else {
	if { $debug } {
...
# following loop
set instances [ array names nios_core_ddr3_p0_ddr_db ]
foreach { inst } $instances {
	if { [ info exists pins ] } {
		# Clean-up stale content
		unset pins
	}
	array set pins $nios_core_ddr3_p0_ddr_db($inst)
	...
	set fpga(tDQS_PHASE_JITTER) [ expr [ get_integer_node_delay -integer $::GLOBAL_nios_core_ddr3_p0_dqs_delay_chain_length -parameters {IO MAX HIGH} -src DQS_PHASE_JITTER -in_fitter ] / 1000.0 ]
	...
	set final_data_input_max_delay [ nios_core_ddr3_p0_round_3dp [ expr $data_input_max_delay + $fpga(tDQS_PSERR) ]]
	set final_data_input_min_delay [ nios_core_ddr3_p0_round_3dp [ expr $data_input_min_delay - $t(CK) / 2.0 + $t(QH_time) - $fpga(tDQS_PSERR) - $tJITper]]

	if { $debug } {
		post_message -type info "SDC: Jitter Parameters"
		post_message -type info "SDC: -----------------"
		post_message -type info "SDC:    DQS Phase: $::GLOBAL_nios_core_ddr3_p0_dqs_delay_chain_length"
	...
	# AFI clock
	set local_pll_afi_clk [ nios_core_ddr3_p0_get_or_add_clock_vseries \
		-target $pll_afi_clock \
		-suffix "afi_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_nios_core_ddr3_p0_pll_mult(PLL_AFI_CLK) \
		-divide_by $::GLOBAL_nios_core_ddr3_p0_pll_div(PLL_AFI_CLK) \
		-phase $::GLOBAL_nios_core_ddr3_p0_pll_phase(PLL_AFI_CLK) ]
	
	# HR PHY clock
	set local_pll_afi_phy_clk [ nios_core_ddr3_p0_get_or_add_clock_vseries \
	...
	# DQ write clock
	set local_pll_dq_write_clk [ nios_core_ddr3_p0_get_or_add_clock_vseries \
		-target $pll_dq_write_clock \
		-suffix "dq_write_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_nios_core_ddr3_p0_pll_mult(PLL_WRITE_CLK) \
		-divide_by $::GLOBAL_nios_core_ddr3_p0_pll_div(PLL_WRITE_CLK) \
		-phase $::GLOBAL_nios_core_ddr3_p0_pll_phase(PLL_WRITE_CLK) ]
	
	# DQS write clock
	set local_pll_write_clk [ nios_core_ddr3_p0_get_or_add_clock_vseries \
		-target $pll_write_clock \
		-suffix "write_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_nios_core_ddr3_p0_pll_mult(PLL_MEM_CLK) \
		-divide_by $::GLOBAL_nios_core_ddr3_p0_pll_div(PLL_MEM_CLK) \
		-phase $::GLOBAL_nios_core_ddr3_p0_pll_phase(PLL_MEM_CLK) ]

	# A/C clock
	set local_pll_addr_cmd_clk [ nios_core_ddr3_p0_get_or_add_clock_vseries \
		-target $pll_ac_clock \
		-suffix "addr_cmd_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_nios_core_ddr3_p0_pll_mult(PLL_ADDR_CMD_CLK) \
		-divide_by $::GLOBAL_nios_core_ddr3_p0_pll_div(PLL_ADDR_CMD_CLK) \
		-phase $::GLOBAL_nios_core_ddr3_p0_pll_phase(PLL_ADDR_CMD_CLK) ]

	# NIOS clock
	set local_pll_avl_clock [ nios_core_ddr3_p0_get_or_add_clock_vseries \
		-target $pll_avl_clock \
		-suffix "avl_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_nios_core_ddr3_p0_pll_mult(PLL_NIOS_CLK) \
		-divide_by $::GLOBAL_nios_core_ddr3_p0_pll_div(PLL_NIOS_CLK) \
		-phase $::GLOBAL_nios_core_ddr3_p0_pll_phase(PLL_NIOS_CLK) ]

	set mem_factor [expr double($::GLOBAL_nios_core_ddr3_p0_pll_mult(PLL_MEM_CLK)) / $::GLOBAL_nios_core_ddr3_p0_pll_div(PLL_MEM_CLK)]
	set avl_factor [expr double($::GLOBAL_nios_core_ddr3_p0_pll_mult(PLL_NIOS_CLK)) / $::GLOBAL_nios_core_ddr3_p0_pll_div(PLL_NIOS_CLK)]
	set write_to_avl_clk_ratio [expr int($mem_factor / $avl_factor)]	
	

	# I/O scan chain clock
	set local_pll_config_clock [ nios_core_ddr3_p0_get_or_add_clock_vseries \
		-target $pll_config_clock \
		-suffix "config_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_nios_core_ddr3_p0_pll_mult(PLL_CONFIG_CLK) \
		-divide_by $::GLOBAL_nios_core_ddr3_p0_pll_div(PLL_CONFIG_CLK) \
		-phase $::GLOBAL_nios_core_ddr3_p0_pll_phase(PLL_CONFIG_CLK) ]	


	# AFI-divided-by-2 clock
	set local_pll_afi_half_clk [ nios_core_ddr3_p0_get_or_add_clock_vseries \
		-target $pll_afi_half_clock \
		-suffix "afi_half_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_nios_core_ddr3_p0_pll_mult(PLL_AFI_HALF_CLK) \
		-divide_by $::GLOBAL_nios_core_ddr3_p0_pll_div(PLL_AFI_HALF_CLK) \
		-phase $::GLOBAL_nios_core_ddr3_p0_pll_phase(PLL_AFI_HALF_CLK) ]

	# Pulse-generator used by DQS tracking
	set local_sampling_clock "${inst}|nios_core_ddr3_p0_sampling_clock"
	...
	foreach { ck_pin } $ck_pins { 
		set_output_delay -add_delay -clock [get_clocks $ck_pin] -max [nios_core_ddr3_p0_round_3dp [expr $t(CK) - $t(DQSS)*$t(CK) - $board(minCK_DQS_skew) ]] $dqs_pins
		set_output_delay -add_delay -clock [get_clocks $ck_pin] -min [nios_core_ddr3_p0_round_3dp [expr $t(DQSS)*$t(CK) - $board(maxCK_DQS_skew) ]] $dqs_pins
	...
	set ac_fr_phase_mod_360 [expr round(2*$::GLOBAL_nios_core_ddr3_p0_pll_phase(3)) % 360]
	set ck_fr_phase_mod_360 [expr round($::GLOBAL_nios_core_ddr3_p0_pll_phase(1)) % 360]
...
if {(($::quartus(nameofexecutable) ne "quartus_fit") && ($::quartus(nameofexecutable) ne "quartus_map"))} {
	set dqs_clocks [nios_core_ddr3_p0_get_all_instances_dqs_pins nios_core_ddr3_p0_ddr_db]
...
add_ddr_report_command "source [list [file join [file dirname [info script]] ${::GLOBAL_nios_core_ddr3_p0_corename}_report_timing.tcl]]"
